...
zonedef_t *read_zone (char *name) {
    ...
    if (name[0] == '/')
        ...
        path = (char*)malloc(strlen(ZONE_PATH) + strlen(name) + 1);
        strcpy(path, ZONE_PATH);
        strcat(path, name);
        free_path = 1;
    }
...
int write_zone (zonedef_t *this_zone) {
    ...
    char *path = (char*)malloc(strlen(ZONE_PATH) + strlen(this_zone->name) + 1);
    strcpy(path, ZONE_PATH);
...
int read_all_zones (uint32_t w, uint32_t h) {
    ...
    return read_zones_in(ZONE_PATH, namebuf, 1023);
}

int read_zones_in (const char *path, char *namebuf, int len) {
    ...
    while ((entry = readdir(zone_dir)) != NULL) {
        ...
        if (elen > len) {
            _show_warning("Load", "Exceeded zone name buffer for entry '%s' at '%s'", entry->d_name, namebuf);
            continue;
        }
        ...
        if (entry->d_type == DT_DIR) {
            ...
            namebuf[sublen++] = '/';
            namebuf[sublen++] = '\0';
            read_zones_in(entry->d_name, namebuf, len - sublen);
            fchdir(dirfd(zone_dir));
            continue;
        }

        zonedef_t *zone = read_zone(namebuf);
        if (zone == NULL) {
