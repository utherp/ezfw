<?php
	...
	class VideoStream {
		...
		public function get_last_index() { return $this->index; }
...
//			if ($this->shmem) @shmop_close($this->shmem);
		...
		private function validate_parameters() {
			...
			$this->filename = abs_path(IMAGE_PATH, constant('IMAGE_'.$this->size.'_'.$this->res));
			if (!is_file($this->filename)) {
				debugger('could not FIND image file for these parameters', 1);
				return false;
			}
...
//			if (!$this->is_image_available()) return false;
		...
		public function image_has_changed() {
			return $this->shmem->image_has_changed();
		}
		/***************************************/
