<?php
    ...
    if (flag_raised(STANDALONE_FLAG)) {
        ...
        $dl_archive = $_SESSION['_login_']->validate_permission('dl_archives');
    }
...
|               Functions                      |
 \********************************************/
    ...
    function construct_header() {
        header("HTTP/1.0 200 OK");      /* \x0d */
        header("Server: arhive_viewer.php/0.1.0");
...
//      $cmd = "ffmpeg -an -ss $seek -i $filename -f mpjpeg -sameq -s 320x240 -r $framerate -";
    ...
    function download_movie($movie) {
        ...
        if ($dl_archive) {
            ...
            for ($i=0; $i<count($times); $i++) $times[$i] = date('Y-m-d H.i', $times[$i]);
            header('Content-Disposition: attachment; filename="('.$times[0] . ')--(' . $times[1] . ').mpg"');
...
/********************************************\
    ...
    if (request_download()) {
        service_logger('Download (' . $filename . ')', $_SESSION['_login_']->get_user()->get_username());
